...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        tie_decode_embeddings = getattr(config, "tie_decode_embeddings", None)
        ...
        if decode_embedding:
            ...
            self.decode_embeddings = nn.Embedding(
                config.decode_vocab_size, config.d_model
            )
        else:
        ...
        if self.adaptor_decode and not self.adaptor_efficient:
            ...
            self.adaptor_embeddings = nn.Embedding(
                config.decode_vocab_size, config.d_model
            )
            self.adaptor = T5Stack(
                decoder_config, self.adaptor_embeddings
            )  # [batch_size, seq_len, emb_dim]
            self.adaptor_linear = nn.Linear(
                config.d_model, config.d_model**2, bias=False
            )
        elif self.adaptor_efficient:
            self.adaptor_embeddings = nn.Parameter(torch.rand(1, 1, config.d_model))
            decoder_layer = nn.TransformerDecoderLayer(
                d_model=config.d_model, nhead=8
            )
            self.adaptor = nn.TransformerDecoder(
                decoder_layer, num_layers=config.adaptor_layer_num
            )
            self.adaptor_linear = nn.Linear(
                config.d_model,
                config.d_model * config.decode_vocab_size,
                bias=False,
            )
        else:
        ...
        if decode_embedding:
            self.lm_head = nn.Linear(
                config.d_model, config.decode_vocab_size, bias=False
            )
        else:
            self.lm_head = nn.Linear(
        ...
        if decode_embedding:
            ...
            ones_indices = torch.ones(seq_length, 1, device=valid_indices.device)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.training and self.Rdrop > 0 and self.Rdrop_only_decoder:
            for i in range(len(var_list)):
                if torch.is_tensor(var_list[i]):
                    var_list[i] = torch.cat(
                        [var_list[i], var_list[i].clone().detach()], dim=0
                    )
            pass
        ...
        def select_valid_embedding(sequence):
            ...
            valid_indices = (
                torch.arange(self.output_vocab_size, device=sequence.device).view(1, -1)
            )
            pos_indices = (
        ...
        if self.adaptor_decode and not self.adaptor_efficient:
            ...
            adaptor_output = adaptor_output * (
                self.model_dim**-0.5
            )  # TODO: What scale should be used
            adaptor_weight = self.adaptor_linear(adaptor_output).reshape(
                adaptor_output.shape[0],
                adaptor_output.shape[1],
                self.model_dim,
                self.model_dim,
            )  # shape(batch_size, sequence_length, config.d_model ** 2)
            ## TODO: add activation?
            lm_head = torch.matmul(
                adaptor_weight, self.lm_head.weight.T
            )  # shape(batch_size, sequence_length, config.d_model, vocab_size)
            lm_logits = torch.matmul(
                sequence_output.unsqueeze(-2), lm_head
            )  # batch matmul, [batch, seq, 1, d_model] * [batch, seq, d_model, vocab_size] = [batch, seq, 1, vocab_size]
        ...
        """
            ...
            lm_head_weight = self_lm_head.weight.T.unsqueeze(0).unsqueeze(
                0
            )  # [1, 1, config.d_model, vocab_size]
            # print("decoder_input_ids",decoder_input_ids)
            decoder_input_embedding = self_decode_embeddings(
                decoder_input_ids
            )  # [batch_size, seq_length, config.d_model]
            batch_size = decoder_input_ids.shape[0]
            seq_length = decoder_input_embedding.shape[1]
            device = decoder_input_embedding.device
            ...
            encode_embedding = self_adaptor_embeddings + torch.zeros(
                batch_size, 1, 1,
                device=decoder_input_embedding.device
            )
        ...
        if self.training:
            self.logit_mask = self.logit_mask.to(lm_logits.device)
            lm_logits = lm_logits + self.logit_mask
        else:
        ...
        if labels is not None:
            if self.Rdrop > 0 and self.training:
                ...
                if self.Rdrop_loss == "KL":
                    p_loss = F.kl_div(
                        F.log_softmax(
                            lm_logits_half.view(-1, lm_logits.size(-1)), dim=-1
                        ),
                        F.softmax(
                            lm_logits_another.view(-1, lm_logits.size(-1)), dim=-1
                        ),
                        reduction="none",
                    )
                    q_loss = F.kl_div(
                        ...
                        F.softmax(lm_logits_half.view(-1, lm_logits.size(-1)), dim=-1),
                        reduction="none",
                    )
                    p_loss = p_loss.sum()
                    q_loss = q_loss.sum()
                    ...
                    neg_logits_1 = sequence_output.transpose(
                        0, 1
                    )  # [sl, bz, vocab_size]
                    ...
                    neg_mask = -1e9 * torch.eye(bz, device=device)
                    neg_logits = neg_logits + neg_mask.unsqueeze(0)
                    neg_logits = F.softmax(
                        neg_logits.view(-1, bz), dim=-1
                    )  # [sl*bz, bz_logits]
                    contrast_labels = torch.cat(
                        [torch.arange(bz // 2, bz, device=device, dtype=torch.long),
                         torch.arange(0, bz // 2, device=device, dtype=torch.long)], dim=-1
                    )
                    contrast_labels = (
                        contrast_labels.unsqueeze(0).repeat(sl, 1).view(-1)
                    )
